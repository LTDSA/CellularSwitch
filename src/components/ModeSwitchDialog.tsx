...
export function ModeSwitchDialog({
  ...
  useEffect(() => {
    if (!open || switchStartedRef.current) return
    switchStartedRef.current = true
    // 每次打开从初始态开始，避免沿用上一次的 success/error 内容。
    setPhase('running')
    setStep('sending')
    setError('')
    setDiagnostics('')
  ...
  }, [open, device, target, moduleService, onSuccess])

  // 手动重新连接：期间不切换界面，对话框保持「切换成功」显示，
  // 直到用户真正选中设备并连接成功才关闭（浏览器弹出的选择框本身
